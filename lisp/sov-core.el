...
(use-package emacs
  ...
  (setq gc-cons-threshold (* 64 1024 1024) ; 设置触发垃圾回收前允许分配的字节数
        ...
        custom-file (locate-user-emacs-file "custom.el") ; 默认情况下, Customize 可能把内容直接写进你的 init.el, 要隔离这种自动生成的代码
        )
  ...
  (setq-default display-line-numbers-type 'relative ; 相对行号
                truncate-lines nil ; 软换行, 拒绝强行截断
                word-wrap t ; 软换行在单词边界断开
                line-spacing nil ;
                line-move-visual t
                )
  ...
  (column-number-mode 1) ; mode-line 不显示行号
